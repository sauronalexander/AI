...
int A_star(vector<vector<Node*> > & maze, int row, int col, int paci,int pacj, int endi,int endj, int & numExp, map<Node*, Node*> & parent, int mincost)
{
...
int multiple(vector<vector<Node*> > & maze, vector<Goal*> & goal, int row, int col, int paci,int pacj, int & numExp)
{
	...
	while(!goal.empty())
	{
		...
		{
			int cost = A_star(maze, row, col, paci, pacj, goal[i]->row, goal[i]->col, numExp, temp_path, goal[temp]->current_cost);
			//cout<<cost<<",";
		...
		A_star(maze, row, col, paci, pacj, goal[temp]->row, goal[temp]->col, numExp, temp_path, goal[temp]->current_cost);
