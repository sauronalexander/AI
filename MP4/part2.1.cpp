...
void Train(vector<vector<double> > & weight, vector<vector<char> > & traindata, vector<int> & trainlabel);
...
void random_5000(vector<int> & index);

void random_5000(vector<int> & index)
{
	while(index.size()<5000)
	{
		int chance = rand()%5000;
		//cout<<chance<<" ";
		vector<int>::iterator it;
		it = find(index.begin(), index.end(), chance);
		if(index.size() == 0)
			index.push_back(chance);
...
int main()
{
	...
	vector<double>  prior;
	Train(weight,  traindata,trainlabel);
...
//		cout<<result[i]<<"   "<<testlabel[i]<<endl;
	...
	cout<<endl;
	cout<<"overall rate: "<<(double)cla/(double)occ<<endl;		
	cout<<endl;
	cout<<endl;
